import "./index.css";

export { default as YeForm } from "./components/YeForm";
export { default as YeFormCustom } from "./components/YeFormCustom";
export { type DynamicInputProps } from "./components/DynamicInput";
export { FormControl } from "./components/ui/form";

export { yeFormTW, getTranslation } from "@yoe-form/shared";
export type {
  YeFormConfig,
  Email,
  EmailHandler,
  RestHandler,
  Language,
} from "@yoe-form/shared";

export { type ControllerRenderProps } from "react-hook-form";
